...
def get_state(env, idx):
    ...
    V2V_abs = (env.V2V_channels_abs[
                   idx * size_platoon, idx * size_platoon + (1 + np.arange(size_platoon - 1))] - 60) / 60.0

    V2I_fast = (env.V2I_channels_with_fastfading[idx * size_platoon, :] - env.V2I_channels_abs[
        idx * size_platoon] + 10) / 35

    V2V_fast = (env.V2V_channels_with_fastfading[idx * size_platoon,
...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        for agent in agents:
            agent.noise = max(0.01, agent.noise * 0.995)
        ...
        for i_step in range(n_step_per_episode):
            ...
            for i in range(n_platoon):
                ...
                action_all_training[i, 2] = np.round(
                    np.clip(((action[2] + 1) / 2) * max_power, 1, max_power))  # power selected by PL
            ...
            state_concat = np.concatenate(state_old_all)
            action_concat = np.concatenate(action_all)
            state_new_concat = np.concatenate(state_new_all)
            reward_l_array = per_total_user[:, i_step]

            memory.store_transition(state_concat, action_concat, global_reward, reward_l_array, state_new_concat,
                                    done)
    ...
    print("Training Finished! Saving data and models...")

    if not os.path.exists('Data'):
        os.makedirs('Data')
    if not os.path.exists('model/marl_ddpg'):
        os.makedirs('model/marl_ddpg')

    np.save('Data/MARL_Reward_Modified.npy', record_global_reward_average)
    np.save('Data/MARL_QoE_Modified.npy', record_QoE_)
    np.save('Data/MARL_V2V_Success_Modified.npy', record_V2V_success_)
