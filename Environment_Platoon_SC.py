...
sys.path.append(os.path.join(project_root, 'DeepSC'))

from DeepSC.models.transceiver import DeepSC
from DeepSC.utils import greedy_decode, PowerNormalize, Channels, SeqtoText, SNR_to_noise
from DeepSC.utils import greedy_decode, PowerNormalize, Channels, SeqtoText, SNR_to_noise, BleuScore
...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, size_platoon, n_RB,
        ...
        project_root = os.path.dirname(os.path.realpath(__file__))
        vocab_path = os.path.join(project_root, 'DeepSC/data/vocab.json')
        with open(vocab_path, 'r', encoding='utf-8') as f:
            vocab = json.load(f)
        self.token_to_idx = vocab['token_to_idx']
        self.idx_to_token = dict(zip(self.token_to_idx.values(), self.token_to_idx.keys()))
        self.num_vocab = len(self.token_to_idx)
        self.pad_idx = self.token_to_idx["<PAD>"]
        self.start_idx = self.token_to_idx["<START>"]
        self.end_idx = self.token_to_idx["<END>"]

        self.deepsc = DeepSC(4, self.num_vocab, self.num_vocab,
                             self.num_vocab, self.num_vocab, 128, 8,
                             512, 0.1).to(self.device)

        checkpoint_path = os.path.join(project_root, 'DeepSC/checkpoints/deepsc-Rayleigh/checkpoint_80.pth')
        self.deepsc.load_state_dict(torch.load(checkpoint_path, map_location=self.device))
        self.deepsc.eval()
        print('DeepSC loaded!')
        # ====== 加载完毕 ======
                        ...
                        if (self.vehicles[i].position[1] <= self.left_lanes[j]) and \
                            ...
                            if (np.random.uniform(0, 1) < self.change_direction_prob):
                                self.vehicles[i].position = [self.vehicles[i].position[0] -
                                                             (delta_distance - (
                                                                         self.left_lanes[j] - self.vehicles[i].position[
                                                                     1])),
                                                             self.left_lanes[j]]
                    ...
                    if change_direction == False:
                        for j in range(len(self.right_lanes)):
                            if (self.vehicles[i].position[1] <= self.right_lanes[j]) and \
                                ...
                                if (np.random.uniform(0, 1) < self.change_direction_prob):
                                    self.vehicles[i].position = [self.vehicles[i].position[0] +
                                                                 (delta_distance + (self.right_lanes[j] -
                    ...
                    for j in range(len(self.left_lanes)):
                        if (self.vehicles[i].position[1] >= self.left_lanes[j]) and \
                            ...
                            if (np.random.uniform(0, 1) < self.change_direction_prob):
                                self.vehicles[i].position = [self.vehicles[i].position[0] -
                                                             (delta_distance - (
                                                                         self.vehicles[i].position[1] - self.left_lanes[
                                                                     j])),
                                                             self.left_lanes[j]]
                                self.vehicles[i].direction = 'l'
                                change_direction = True
                    ...
                    if change_direction == False:
                        for j in range(len(self.right_lanes)):
                            if (self.vehicles[i].position[1] >= self.right_lanes[j]) and \
                                ...
                                if (np.random.uniform(0, 1) < self.change_direction_prob):
                                    self.vehicles[i].position = [self.vehicles[i].position[0] +
                                                                 (delta_distance + (self.vehicles[i].position[1] -
                                                                                    self.right_lanes[j])),
                                                                 self.right_lanes[j]]
                                    self.vehicles[i].direction = 'r'
                                    change_direction = True
                        ...
                        if (self.vehicles[i].position[0] <= self.up_lanes[j]) and \
                            ...
                            if (np.random.uniform(0, 1) < self.change_direction_prob):
                                self.vehicles[i].position = [self.up_lanes[j], self.vehicles[i].position[1] +
                                                             (delta_distance - (
                                                                         self.up_lanes[j] - self.vehicles[i].position[
                                                                     0]))]
                                change_direction = True
                            ...
                            if (self.vehicles[i].position[0] <= self.down_lanes[j]) and \
                                ...
                                if (np.random.uniform(0, 1) < self.change_direction_prob):
                                    self.vehicles[i].position = [self.down_lanes[j], self.vehicles[i].position[1] -
                                                                 (delta_distance - (self.down_lanes[j] -
                    ...
                    for j in range(len(self.up_lanes)):
                        if (self.vehicles[i].position[0] >= self.up_lanes[j]) and \
                            ...
                            if (np.random.uniform(0, 1) < self.change_direction_prob):
                                self.vehicles[i].position = [self.up_lanes[j], self.vehicles[i].position[1] +
                                                             (delta_distance - (
                                                                         self.vehicles[i].position[0] - self.up_lanes[
                                                                     j]))]
                                change_direction = True
                                self.vehicles[i].direction = 'u'
                                break
                    if change_direction == False:
                        for j in range(len(self.down_lanes)):
                            if (self.vehicles[i].position[0] >= self.down_lanes[j]) and \
                                ...
                                if (np.random.uniform(0, 1) < self.change_direction_prob):
                                    self.vehicles[i].position = [self.down_lanes[j], self.vehicles[i].position[1] -
                                                                 (delta_distance - (self.vehicles[i].position[0] -
                                                                                    self.down_lanes[j]))]
                                    change_direction = True
                                    self.vehicles[i].direction = 'd'
                                    break
    ...
    def Compute_Performance_Reward_Train(self, platoons_actions, V2V_length):
        sub_selection = platoons_actions[:, 0].astype('int').reshape(int(self.n_Veh / self.size_platoon),
                                                                     1)  # channel_selection_part
        platoon_decision = platoons_actions[:, 1].astype('int').reshape(int(self.n_Veh / self.size_platoon),
                                                                        1)  # platoon selection Intra/Inter platoon communication
        ...
        self.platoon_V2V_Interference = np.zeros(
            [int(self.n_Veh / self.size_platoon), self.size_platoon - 1])  # V2V interferences
        ...
        for i in range(self.n_RB):
            ...
            for j in range(len(indexes)):
                for k in range(len(indexes)):
                    if indexes[j, 0] != indexes[k, 0] and platoon_decision[
                        indexes[j, 0], 0] == 0:  # platoon j has selected the inter-platoon communication
                    ...
                    if indexes[j, 0] != indexes[k, 0] and platoon_decision[
                        indexes[j, 0], 0] == 1:  # platoon j has selected the intra-platoon communication
                        # if not self.active_links[indexes[k, 0]] and platoon_decision[indexes[k, 0], 0] == 1:
                        #     continue
        ...
        for i in range(self.n_RB):
            ...
            for j in range(len(indexes)):
                if platoon_decision[indexes[j, 0], 0] == 0:
                    self.platoon_V2I_Signal[indexes[j, 0]] = 10 ** ((power_selection[indexes[j, 0], 0] -
                                                                     self.V2I_channels_with_fastfading[
                                                                         indexes[j, 0] * self.size_platoon, i] +
                                                                     self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
                elif platoon_decision[indexes[j, 0], 0] == 1:
                    for l in range(self.size_platoon - 1):
                        self.platoon_V2V_Signal[indexes[j, 0], l] += 10 ** ((power_selection[indexes[j, 0], 0] -
                                                                             self.V2V_channels_with_fastfading[
                                                                                 indexes[j, 0] * self.size_platoon,
                                                                                 indexes[j, 0] * self.size_platoon + (
                                                                                             l + 1), i] +
                                                                             2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        V2I_Rate = np.log2(
            1 + np.divide(self.platoon_V2I_Signal, (self.platoon_V2I_Interference + self.sig2)))  # 每秒每赫兹（bps/Hz）
        V2V_Rate = np.log2(
            1 + np.divide(self.platoon_V2V_Signal, (self.platoon_V2V_Interference + self.sig2)))  # 每秒每赫兹（bps/Hz）
        ...
        si = 0.3 + (0.5 - 0.3) * np.random.rand(int(self.n_Veh / self.size_platoon), 1)
        lamda = np.random.normal(5, 0.5, (int(self.n_Veh / self.size_platoon), 1))

        QoE = np.zeros(int(self.n_Veh / self.size_platoon))
            ...
            for i in range(int(self.n_Veh / self.size_platoon)):
                if platoon_decision[i, 0] == 1:
                    ...
                    for j in range(self.size_platoon - 1):
                        QoE1 += w / (1 + np.exp(beta_S[i] * (phi_S[i] - V2V_Rate_semantic[i, j] / 1000))) + (1 - w) / (
                                1 + np.exp(lamda[i] * (si[i] - V2V_semantic[i, j])))
                    QoE[i] = QoE1 / 4
        ...
        if V2I_number == 1:
            for i in range(int(self.n_Veh / self.size_platoon)):
                ...
                if platoon_decision[i, 0] == 1:
                    ...
                    QoE2 = 0
                    for j in range(self.size_platoon - 1):
                        QoE2 += w / (1 + np.exp(beta_S[i] * (phi_S[i] - V2V_Rate_semantic[i, j] / 1000))) + (1 - w) / (
                                1 + np.exp(lamda[i] * (si[i] - V2V_semantic[i, j])))
                    QoE[i] = QoE2 / 4
        ...
        if V2I_number == 2:
            ...
            V2I_semantic[index[0, 0]] = V2I_semantic[index[1, 0]] = V2I_list1[
                V2I_SINR_index[index[0, 0]], V2I_SINR_index[index[1, 0]]]

            V2I_Rate_semantic[index[0, 0]] = 2 / (symbol_text[int(V2I_length[index[0, 0], 0] * 5)] / self.bandwidth)
            V2I_Rate_semantic[index[1, 0]] = (4 * 197) / (
                    symbol_image[int(V2I_length[index[1, 0], 0] * 5)] / self.bandwidth)
            ...
            for i in range(int(self.n_Veh / self.size_platoon)):
                if platoon_decision[i, 0] == 1:
                    ...
                    QoE3 = 0
                    for j in range(self.size_platoon - 1):
                        QoE3 += w / (1 + np.exp(beta_S[i] * (phi_S[i] - V2V_Rate_semantic[i, j] / 1000))) + (1 - w) / (
                                1 + np.exp(lamda[i] * (si[i] - V2V_semantic[i, j])))
                    QoE[i] = QoE3 / 4
        ...
        if V2I_number == 3:
            ...
            V2I_semantic[index[0, 0]] = V2I_semantic[index[1, 0]] = V2I_list1[
                V2I_SINR_index[index[0, 0]], V2I_SINR_index[index[1, 0]]]
            V2I_semantic[index[2, 0]] = single_table_data[
                int(V2I_length[index[2, 0], 0] * 20), V2I_SINR_index[index[2, 0]]]
            ...
            QoE3 = w / (1 + np.exp(
                beta_S[index[2, 0]] * (phi_S[index[2, 0]] - V2I_Rate_semantic[index[2, 0]] / 1000))) + \
            ...
            for i in range(int(self.n_Veh / self.size_platoon)):
                if platoon_decision[i, 0] == 1:
                    ...
                    QoE4 = 0
                    for j in range(self.size_platoon - 1):
                        QoE4 += w / (1 + np.exp(beta_S[i] * (phi_S[i] - V2V_Rate_semantic[i, j] / 1000))) + (1 - w) / (
                                1 + np.exp(lamda[i] * (si[i] - V2V_semantic[i, j])))
                    QoE[i] = QoE4 / 4
    ...
    def act_for_training(self, actions, V2V_length):
        ...
        V2V_demand_semantic, reward_elements_semantic, V2I_semantic, \
            V2V_semantic, interplatoon_rate_semantic, intraplatoon_rate_semantic, QoE = self.Compute_Performance_Reward_Train(
            action_temp, V2V_length)

        V2V_success = 1 - np.sum(self.active_links_semantic) / (
            int(self.n_Veh / self.size_platoon))  # V2V success rates
        QoE = np.array(QoE).flatten()
        for i in range(int(self.n_Veh / self.size_platoon)):
            w_penalty = 1
            per_user_reward[i] = -(w_penalty) * (V2V_demand_semantic[i] / self.V2V_demand_size_semantic) + 10 * QoE[i]
    ...
    def new_random_game(self, n_Veh=0):
        ...
        self.V2V_demand_semantic = self.V2V_demand_size_semantic * np.ones(int(self.n_Veh / self.size_platoon),
                                                                           dtype=np.float16) / 20
        self.individual_time_limit_semantic = self.time_slow * np.ones(int(self.n_Veh / self.size_platoon),
                                                                       dtype=np.float16)
    ...
    def get_SINR_for_test(self, platoons_actions, V2V_length):
        sub_selection = platoons_actions[:, 0].astype('int').reshape(int(self.n_Veh / self.size_platoon), 1)
        platoon_decision = platoons_actions[:, 1].astype('int').reshape(int(self.n_Veh / self.size_platoon), 1)
        power_selection = platoons_actions[:, 2].reshape(int(self.n_Veh / self.size_platoon), 1)

        V2I_Signal = np.zeros(int(self.n_Veh / self.size_platoon))
        V2I_Interference = np.zeros(int(self.n_Veh / self.size_platoon))
        V2V_Signal = np.zeros([int(self.n_Veh / self.size_platoon), self.size_platoon - 1])
        V2V_Interference = np.zeros([int(self.n_Veh / self.size_platoon), self.size_platoon - 1])
        ...
        for i in range(self.n_RB):
            ...
            for j in range(len(indexes)):
                for k in range(len(indexes)):
                    if indexes[j, 0] != indexes[k, 0] and platoon_decision[indexes[j, 0], 0] == 0:
                        V2I_Interference[indexes[j, 0]] += \
                            10 ** ((power_selection[indexes[k, 0], 0] - self.V2I_channels_with_fastfading[
                                indexes[k, 0] * self.size_platoon, i] +
                                    self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
                    if indexes[j, 0] != indexes[k, 0] and platoon_decision[indexes[j, 0], 0] == 1:
                        for l in range(self.size_platoon - 1):
                            V2V_Interference[indexes[j, 0], l] += \
                                10 ** ((power_selection[indexes[k, 0], 0] - self.V2V_channels_with_fastfading[
                                    indexes[k, 0] * self.size_platoon, indexes[j, 0] * self.size_platoon + (l + 1), i] +
                                        2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        for i in range(self.n_RB):
            ...
            for j in range(len(indexes)):
                if platoon_decision[indexes[j, 0], 0] == 0:
                    V2I_Signal[indexes[j, 0]] = 10 ** (
                            (power_selection[indexes[j, 0], 0] - self.V2I_channels_with_fastfading[
                                indexes[j, 0] * self.size_platoon, i] +
                             self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
                elif platoon_decision[indexes[j, 0], 0] == 1:
                    for l in range(self.size_platoon - 1):
                        V2V_Signal[indexes[j, 0], l] += 10 ** (
                                (power_selection[indexes[j, 0], 0] - self.V2V_channels_with_fastfading[
                                    indexes[j, 0] * self.size_platoon, indexes[j, 0] * self.size_platoon + (
                                            l + 1), i] +
                                 2 * self.vehAntGain - self.vehNoiseFigure) / 10)

        # 计算 SINR (dB)
        V2I_SINR = 10 * np.log10(V2I_Signal / (V2I_Interference + self.sig2) + 1e-9)
        V2V_SINR = 10 * np.log10(V2V_Signal / (V2V_Interference + self.sig2) + 1e-9)

        # 限制范围与原代码一致
        V2I_SINR = np.clip(V2I_SINR, -10, 20)
        V2V_SINR = np.clip(V2V_SINR, -10, 20)

        return V2I_SINR, V2V_SINR

    def semantic_transmission(self, original_text, sinr_db):
        ...
        tokens = my_tokenizer(original_text)

        unk_idx = self.token_to_idx.get('<UNK>', self.pad_idx)
        token_ids = [self.token_to_idx.get(t, unk_idx) for t in tokens]
        src = torch.LongTensor([token_ids]).to(self.device)  # shape: [1, seq_len]

        noise_std = SNR_to_noise(np.array(sinr_db))

        max_length = 30
        with torch.no_grad():
            output_ids = greedy_decode(self.deepsc, src, noise_std, max_length,
                                       self.pad_idx, self.start_idx, 'Rayleigh')

        StoT = SeqtoText(self.token_to_idx, self.end_idx)
        sentences = output_ids.cpu().numpy().tolist()
        reconstructed_text = list(map(StoT.sequence_to_text, sentences))[0]
...
def compute_true_semantic_similarity(original_text, reconstructed_text):
    words1 = set(original_text.strip().lower().split())
    words2 = set(reconstructed_text.strip().lower().split())
...
def generate_sentence_pair(env, sinr_db):
    base_sentences = [
        "The platoon leader is accelerating to merge into the highway."
        , "Vehicle ahead is braking suddenly, reduce speed immediately."
        , "Traffic light will turn green in five seconds."
        , "Road construction ahead, change to left lane."
        , "Emergency vehicle approaching from behind, yield right."
        , "Maintain current speed, no obstacles detected."
        , "Platoon member three is leaving the formation."
        , "Slow down, sharp curve in two hundred meters."
    ]
    original = random.choice(base_sentences)

    original_processed = original.lower().strip()
    if not original_processed.endswith('.'):
        original_processed += '.'

    reconstructed_processed = env.semantic_transmission(original_processed, sinr_db)

    reconstructed_processed = reconstructed_processed.replace('<START>', '').replace('<END>', '').strip()
    reconstructed_processed = re.sub(r'[^a-zA-Z\s]', '', reconstructed_processed)
    if not reconstructed_processed:
        reconstructed_processed = "signal lost"

    return original, reconstructed_processed
